...
Mymodel=multinom(Species~PC1,data=Trg)
summary(Mymodel)
...
library(ggplot2)
help(ggplot2)
c=ggplot(iris,aes(Petal.Length, Petal.Width, color=iris$Species))+geom_point(alpha=0.4,size=3.5)+geom_point(col=clustercut)+scale_color_manual(values=c('black','red','green'))
c
...
c1=ggplot(iris,aes(Petal.Length, Petal.Width, color=iris$Species))+geom_point(alpha=0.4,size=3.5)+geom_point(col=clustercut1)+scale_color_manual(values=c('black','red','green'))
c1
clustercut1

TitanicData=read.csv(file.choose(),header=T,sep=",")
summary(TitanicData)
names(TitanicData)
...
Tree=chaid(Survived ~ Pclass + Sex + Age + SibSp + Parch + Fare + Embarked, data=TitanicData)


library(rpart)
fit = rpart(Survived ~ Pclass + Sex + Age + SibSp + Parch + Fare + Embarked, data=TitanicData,method = "class")
plot(fit)
text(fit)
...
fancyRpartPlot(fit)
Prediction = predict(fit, TitanicData, type="class")
Prediction
